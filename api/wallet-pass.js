...
export default async function handler(req, res) {
  ...
  try {
    ...
    const genericObject = {
      ...
      state: "ACTIVE",
      logo: {
        sourceUri: { uri: `${HUB_URL}/icon-192.png` },
      },
    ...
    if (!insertRes.ok) {
      ...
      // A 409 means it already exists — that's expected on a re-add, patch it.
      // Anything else (400, 404, etc.) is a real problem with the create
      // itself — report THAT, since a follow-up patch failing is just a
      // symptom of the object never having been created in the first place.
      let insertErrJson = null;
      try { insertErrJson = JSON.parse(insertErrText); } catch (e) { /* not JSON */ }
      const isAlreadyExists = insertRes.status === 409 || (insertErrJson && insertErrJson.error && insertErrJson.error.code === 409);

      if (!isAlreadyExists) {
        return res.status(500).json({ error: `Could not create the pass (status ${insertRes.status}): ${insertErrText}` });
      }
      ...
      if (!patchRes.ok) {
        ...
        return res.status(500).json({ error: `Object existed but could not be updated: ${err}` });
      }
    }
