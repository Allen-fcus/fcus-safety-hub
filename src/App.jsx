...
function HSStandardsScreen({ user }) {
  ...
  return (
    <div className="px-4 pt-4 pb-8 flex flex-col" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      ...
      <div className="mb-3">
        <div className="text-[14px]" style={{ fontFamily: "Oswald, sans-serif", fontWeight: 600 }}>{HS_STANDARDS_ITEM.title}</div>
        <div className="text-[11px]" style={{ color: STEEL }}>{HS_STANDARDS_ITEM.pages} pages · Same for every project</div>
      </div>
      {HS_STANDARDS_ITEM.link ? (
        <div className="flex-1 rounded-md overflow-hidden border" style={{ borderColor: "#E4E2DA", minHeight: 500 }}>
          <iframe src={HS_STANDARDS_ITEM.link} className="w-full h-full" style={{ border: "none", minHeight: 500 }} title={HS_STANDARDS_ITEM.title} />
        </div>
      ) : (
        <div className="flex-1 flex items-center justify-center py-12" style={{ color: STEEL }}>No document set up yet.</div>
      )}
      <button
        onClick={() => window.open(HS_STANDARDS_ITEM.link, "_blank")}
