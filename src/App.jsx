...
function ToolboxScreen({ user, go, activeProjectId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      await loadProjectMap();
      const projectUuid = projectIdForSlug(activeProjectId);
      if (!projectUuid) { if (!cancelled) { setTalks([]); setLoading(false); } return; }
      const { data, error } = await supabase
        .from("toolbox_talks")
        .select("*")
        .eq("project_id", projectUuid)
  ...
  const openTalk = async (t) => {
    setPlaying(t);
    setAcked(false);
    setAlreadyAcked(false);
    if (user && user.sessionToken) {
      const { data } = await supabase.rpc("has_acknowledged_toolbox_talk", { p_token: user.sessionToken, p_talk_id: t.id });
  ...
  const confirmReviewed = async () => {
    ...
    setAcking(true);
    const { data } = await supabase.rpc("acknowledge_toolbox_talk", { p_token: user.sessionToken, p_talk_id: playing.id });
    setAcking(false);
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="flex items-center justify-between mb-3">
        <div className="text-[12px]" style={{ color: STEEL }}>
          Short weekly briefings. Watch, then confirm you reviewed it.
        </div>
        {user && user.canAddPersonnel && (
      ...
      <div className="space-y-2">
        {talks.map((t, i) => (
          ...
          >
            <div className="w-14 h-14 rounded-md flex items-center justify-center flex-shrink-0" style={{ background: INK }}>
              {t.docLink && !t.videoLink ? <FileText size={20} color={AMBER} /> : <Play size={20} color={AMBER} fill={AMBER} />}
            </div>
        ...
        {talks.length === 0 && !loading && (
          <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>No toolbox talks posted for this project yet.</div>
        )}
      </div>
          ...
          <div className="bg-white rounded-md w-full max-w-sm overflow-hidden flex flex-col" style={{ maxHeight: "85vh" }}>
            {playing.docLink ? (
              <div className="flex-1" style={{ minHeight: 300 }}>
                <iframe src={playing.docLink} className="w-full h-full" style={{ border: "none", minHeight: 300 }} title={playing.title} />
              </div>
            ) : playing.videoLink ? (
              ...
              <div className="aspect-video flex items-center justify-center" style={{ background: INK }}>
                <div className="text-center">
                  <FileText size={36} color={AMBER} className="mx-auto" />
                  <div className="text-white/60 text-[11px] mt-2">No document or video link set for this talk yet.</div>
                </div>
              </div>
            )}
            <div className="p-3 flex-shrink-0">
              ...
              {playing.docLink && (
                <button
                  onClick={() => window.open(playing.docLink, "_blank")}
              ...
              {!user || !user.sessionToken ? (
                <div className="text-[11px] text-center py-2" style={{ color: STEEL }}>Log in with your badge to confirm you reviewed this.</div>
              ) : acked || alreadyAcked ? (
                ...
                <button
                  ...
                  style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}
                >
                  {acking ? "Confirming…" : "I Have Reviewed & Understand This Topic"}
                </button>
              )}
            </div>
          </div>
...
function ToolboxTalkEditor({ talk, user, onDone }) {
  const [title, setTitle] = useState(talk ? talk.title : "");
  const [lengthLabel, setLengthLabel] = useState(talk ? talk.length_label || "" : "");
  const [videoLink, setVideoLink] = useState(talk ? talk.video_link || "" : "");
  const [docLink, setDocLink] = useState(talk ? talk.doc_link || "" : "");
  ...
  const submit = async () => {
    ...
    setSaving(true);
    setError("");
    const { data, error: rpcError } = talk
      ? await supabase.rpc("update_toolbox_talk", {
          p_token: user.sessionToken, p_talk_id: talk.id,
          p_title: title.trim(), p_length_label: lengthLabel.trim(), p_video_link: videoLink.trim(), p_doc_link: docLink.trim(),
        })
      : await supabase.rpc("add_toolbox_talk", {
  ...
  return (
    <div className="bg-white rounded-md p-3 border mb-3" style={{ borderColor: "#E4E2DA" }}>
      ...
      <label className="text-[10px] uppercase font-bold" style={{ color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>Length (e.g. "5 min read")</label>
      <input value={lengthLabel} onChange={(e) => setLengthLabel(e.target.value)} className="w-full mt-1 mb-2 rounded-md border px-2.5 py-1.5 text-[13px]" style={{ borderColor: "#C9C6BC" }} />

      <label className="text-[10px] uppercase font-bold" style={{ color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>Document Link (PDF/Word — Google Drive "Anyone with the link" preview URL)</label>
      ...
      <label className="text-[10px] uppercase font-bold" style={{ color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>Video Link (optional, if there's also a video)</label>
      ...
      <div className="flex gap-2">
        <button onClick={submit} disabled={saving} className="flex-1 text-[11px] font-bold uppercase py-2 rounded-sm disabled:opacity-50" style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}>
          {saving ? "Saving…" : talk ? "Save Changes" : "Post Talk"}
        </button>
...
function ToolboxAckList({ talk, user, onClose }) {
  const [list, setList] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      const { data } = await supabase.rpc("list_toolbox_acknowledgments", { p_token: user.sessionToken, p_talk_id: talk.id });
      setList(data || []);
      setLoading(false);
    }
    load();
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 px-4">
      <div className="bg-white rounded-md w-full max-w-sm overflow-hidden max-h-[80%] flex flex-col">
        <div className="flex items-center justify-between p-3 border-b flex-shrink-0" style={{ borderColor: "#E4E2DA" }}>
          <div className="text-[13px]" style={{ fontFamily: "Oswald, sans-serif", fontWeight: 600 }}>{talk.title}</div>
          <button onClick={onClose}><X size={18} color={STEEL} /></button>
        </div>
        <div className="p-3 overflow-y-auto">
          {loading && <div className="text-[12px]" style={{ color: STEEL }}>Loading…</div>}
          {!loading && list.length === 0 && (
            <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>Nobody has confirmed this talk yet.</div>
          )}
          <div className="space-y-2">
            {list.map((a, i) => (
...
function ManageToolboxScreen({ user }) {
  ...
  const load = async () => {
    setLoading(true);
    const { data, error } = await supabase.rpc("list_toolbox_admin", { p_token: user.sessionToken });
    setTalks(error || !data ? [] : data);
    setLoading(false);
  };
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="text-[12px] mb-3" style={{ color: STEEL }}>
        Post this week's topic, and see exactly who's confirmed reviewing each one — by name and employer.
      </div>
      ...
      {showNew && <ToolboxTalkEditor user={user} onDone={() => { setShowNew(false); load(); }} />}

      {loading && <div className="text-[12px] mb-2" style={{ color: STEEL }}>Loading…</div>}

      {talks.map((t) =>
        editingId === t.id ? (
          <ToolboxTalkEditor key={t.id} talk={t} user={user} onDone={() => { setEditingId(null); load(); }} />
        ) : (
          <div key={t.id} className="bg-white rounded-md p-3 border mb-2" style={{ borderColor: "#E4E2DA" }}>
            <div className="text-[13px]" style={{ fontFamily: "Oswald, sans-serif", fontWeight: 600 }}>{t.title}</div>
            <div className="text-[11px] mt-0.5" style={{ color: STEEL }}>{t.length_label || "—"} · Posted {new Date(t.posted_at).toLocaleDateString()}</div>
            <div className="text-[11px] mt-1 font-bold" style={{ color: GOLD }}>{t.ack_count} confirmed reviewed</div>
            <div className="flex gap-2 mt-2">
              <button onClick={() => setViewingAcks(t)} className="flex-1 text-[10px] font-bold uppercase py-1.5 rounded-sm" style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}>Who Confirmed</button>
              <button onClick={() => setEditingId(t.id)} className="flex-1 text-[10px] font-bold uppercase py-1.5 rounded-sm border" style={{ borderColor: "#C9C6BC", color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>Edit</button>
              <button onClick={() => remove(t.id)} className="flex-1 text-[10px] font-bold uppercase py-1.5 rounded-sm border" style={{ borderColor: ALERT, color: ALERT, fontFamily: "IBM Plex Mono, monospace" }}>Delete</button>
            </div>
          </div>
        )
      )}
      {!loading && talks.length === 0 && !showNew && (
        <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>No toolbox talks yet — post the first one above.</div>
      )}

      {viewingAcks && <ToolboxAckList talk={viewingAcks} user={user} onClose={() => setViewingAcks(null)} />}
    </div>
  );
}
