...
function FillFormModal({ form, onClose, user }) {
  ...
  const doSubmit = async () => {
    ...
    // training certification) — upload it alongside the submission.
    if (form.requiresProof && proofFile) {
      const path = `proof-${Date.now()}-${proofFile.name.replace(/[^a-zA-Z0-9.\-_]/g, "_")}`;
      const { error: proofUploadError } = await supabase.storage.from("documents").upload(path, proofFile);
      if (!proofUploadError) {
        const { data: urlData } = supabase.storage.from("documents").getPublicUrl(path);
        proofDocUrl = urlData && urlData.publicUrl ? urlData.publicUrl : null;
      }
    }
    ...
    if (recipientEmail) {
      const linksText = [
        ...
        proofDocUrl ? `Supporting Document: ${proofDocUrl}` : null,
      ].filter(Boolean).join("\n");
    ...
    if (user && user.sessionToken) {
      try {
        await supabase.rpc("submit_form", {
          ...
          p_data: { ...standardValues, fields: values, statusValues, choiceValues, stepValues, filledPdfUrl, proofDocUrl },
          p_signature: null,
        });
      } catch (e) {
        ...
        <div className="p-4 space-y-4 overflow-y-auto">
          ...
          {form.requiresProof && (
            <div className="mb-4">
              ...
              <input ref={proofInputRef} type="file" accept="image/*,.pdf" className="hidden" onChange={(e) => {
                const f = e.target.files && e.target.files[0];
                if (!f) return;
                setProofFile(f);
              ...
              {proofFileName ? (
                <div className="flex items-center justify-between mt-1 rounded-md border px-2.5 py-1.5" style={{ borderColor: "#C9C6BC" }}>
                  <span className="text-[12px] truncate">{proofFileName}</span>
                  <button onClick={() => { setProofFile(null); setProofFileName(""); }}><X size={14} color={STEEL} /></button>
                </div>
              ) : (
                <button onClick={() => proofInputRef.current && proofInputRef.current.click()} className="w-full mt-1 rounded-md border-2 border-dashed py-3 flex flex-col items-center gap-1" style={{ borderColor: "#C9C6BC" }}>
                  <ImagePlus size={18} color={STEEL} />
                  <span className="text-[11px]" style={{ color: STEEL }}>Attach a photo or PDF</span>
                </button>
              )}
            </div>
          )}
          ...
          <button
            disabled={sigEmpty || submitting || (form.requiresProof && form.proofRequired !== false && !proofFile)}
            onClick={doSubmit}
      ...
      {submissions.map((s) => (
        <div key={s.id} className="bg-white rounded-md p-3 border mb-2" style={{ borderColor: "#E4E2DA" }}>
          ...
          <div className="flex gap-2 mt-2">
            ...
            {s.proof_doc_url && (
              <button onClick={() => window.open(s.proof_doc_url, "_blank")} className="flex-1 text-[10px] font-bold uppercase py-1.5 rounded-sm border" style={{ borderColor: "#C9C6BC", color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>
                View Proof Doc
              </button>
            )}
          </div>
