...
function HomeScreen({ go, progress, user, activeProjectId }) {
  ...
  const sections = [
    ...
    { key: "hsstandards", label: "H&S Minimum Standards", desc: "Official company-wide safety standards", icon: BookOpen, count: "Always visible" },
    { key: "safetyplan", label: "Safety Plan", desc: "This project's safety plan", icon: ClipboardCheck, count: `${fallbacks.safetyPlan.length} document${fallbacks.safetyPlan.length !== 1 ? "s" : ""}` },
    ...
    { key: "laborlaws", label: "TX & Federal Labor Laws", desc: "Required postings, English & Spanish", icon: ShieldAlert, count: "Always available" },
...
function LaborLawScreen({ user, go, activeProjectId }) {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="flex items-center justify-between mb-3">
        <div className="text-[12px]" style={{ color: STEEL }}>
          Required Texas and Federal labor law postings. Available to everyone — no login required.
        </div>
        {user && user.canAddPersonnel && (
      ...
      <div className="space-y-2">
        ...
        {!loading && Object.keys(grouped).length === 0 && (
          <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>No labor law postings for this project yet.</div>
        )}
      </div>
      ...
      {expanded && (
        <div className="px-3 pb-3">
          ...
          {!user || !user.sessionToken ? (
            <div className="text-[11px] text-center py-2" style={{ color: STEEL }}>Log in with your badge to confirm you reviewed these.</div>
          ) : isConfirmed ? (
            <div className="w-full flex items-center justify-center gap-2 rounded-sm py-2.5 text-[12px] font-bold uppercase" style={{ background: "#E9F6EC", color: "#1E7A34", fontFamily: "IBM Plex Mono, monospace" }}>
              <CheckCircle2 size={15} /> Confirmed Reviewed
            </div>
          ) : (
            <button
              ...
              style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}
            >
              {acking ? "Confirming…" : "I Have Reviewed & Understand These Topics"}
            </button>
          )}
        </div>
      )}
...
function ToolboxScreen({ user, go, activeProjectId }) {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="flex items-center justify-between mb-3">
        <div className="text-[12px]" style={{ color: STEEL }}>
          This week's set is open below. Older weeks are collapsed — tap to review them anytime.
        </div>
        {user && user.canAddPersonnel && (
      ...
      {!loading && weeks.length === 0 && (
        <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>No toolbox topics posted for this project yet.</div>
      )}
    </div>
  );
}
...
function WeeklyReportScreen({ user, go, activeProjectId }) {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="flex items-center justify-between mb-3">
        <div className="text-[12px]" style={{ color: STEEL }}>
          Weekly lessons learned, posted as PDFs — most recent first.
        </div>
        {user && user.canAddPersonnel && (
      ...
      <div className="space-y-2">
        ...
        {!loading && reports.length === 0 && (
          <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>No weekly reports posted for this project yet.</div>
        )}
      </div>
