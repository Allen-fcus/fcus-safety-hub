...
async function fillDrugScreenPdf({ templateUrl, companyName, companyAddress, companyCityStateZip, recipientName, date, employeeNames, contactName, contactPhone, logoDataUrl }) {
  ...
  try {
    ...
    // "Your Logo Here" is emptied out specifically so a logo image can be
    // drawn over it — pdf-lib can sometimes choke flattening an emptied
    // text field. Remove just that one field and retry flattening the rest.
    try {
      form.removeField(form.getTextField("Your Logo Here"));
      form.flatten();
    } catch (e2) {
      // Still couldn't flatten — leave the fields filled but still
      // interactive rather than fail the whole PDF generation.
    }
  }
  ...
  if (logoDataUrl) {
    ...
    if (!isPng && !isJpeg) {
      ...
      throw new Error(
        `The logo file is a "${mimeMatch ? mimeMatch[1] : "unknown"}" format, which isn't supported for the fillable PDF — please use a PNG or JPG image instead (a screenshot of the logo, or a re-saved copy, works well; a photo taken directly on an iPhone is often HEIC format, which isn't supported here).`
      );
    }
    let imgBytes;
    try {
      ...
      throw new Error(`Could not read the uploaded logo file: ${e.message}`);
    }
    let logoImage;
    try {
      logoImage = isJpeg ? await pdfDoc.embedJpg(imgBytes) : await pdfDoc.embedPng(imgBytes);
    } catch (e) {
      throw new Error(`Could not embed the logo image into the PDF (${e.message}). Try a different PNG or JPG file.`);
    }
    const page = pdfDoc.getPage(0);
    // "Your Logo Here" field rect: [191.336, 720.168, 411.336, 742.168]
