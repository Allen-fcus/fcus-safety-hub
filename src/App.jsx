...
function ToolboxDocEditor({ weekId, user, onDone }) {
  ...
  const submit = async () => {
    ...
    const { data, error: rpcError } = await supabase.rpc("add_toolbox_talk_to_week", {
      ...
      p_title: title.trim(), p_doc_link: docLink.trim(), p_video_link: videoLink.trim(),
    });
    setSaving(false);
  ...
  return (
    <div className="bg-white rounded-md p-3 border mb-2" style={{ borderColor: "#E4E2DA" }}>
      ...
      <label className="text-[10px] uppercase font-bold" style={{ color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>Document Link (PDF/Word — Drive "Anyone with the link" preview URL)</label>
      <input value={docLink} onChange={(e) => setDocLink(e.target.value)} placeholder="https://drive.google.com/file/d/…/preview" className="w-full mt-1 mb-2 rounded-md border px-2.5 py-1.5 text-[13px]" style={{ borderColor: "#C9C6BC" }} />
