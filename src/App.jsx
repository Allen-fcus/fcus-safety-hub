    ...
    if (user && user.sessionToken) {
      try {
        await supabase.rpc("submit_form", {
          ...
          p_form_title: form.title,
          p_data: { ...standardValues, fields: values, statusValues, choiceValues, stepValues, filledPdfUrl, proofDocUrls },
          p_signature: null,
        });
      } catch (e) {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="flex items-center justify-between mb-3">
        ...
        {user && user.canAddPersonnel && (
          <button onClick={() => go("managesubmittedforms")} className="text-[10px] font-bold uppercase px-2 py-1 rounded-sm border flex-shrink-0 ml-2" style={{ borderColor: "#C9C6BC", color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>
            Submissions
          </button>
        )}
      </div>
...
function SubmittedFormsScreen({ user }) {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="text-[12px] mb-3" style={{ color: STEEL }}>
        Every form submitted, with the actual completed document — this works even if email delivery didn't.
      </div>
      {loading && <div className="text-[12px] mb-2" style={{ color: STEEL }}>Loading…</div>}
      {submissions.map((s) => (
        <div key={s.id} className="bg-white rounded-md p-3 border mb-2" style={{ borderColor: "#E4E2DA" }}>
      ...
      {!loading && submissions.length === 0 && (
        <div className="text-[12px] text-center py-6" style={{ color: STEEL }}>No submissions yet.</div>
      )}
    </div>
  );
}
...
const BASE_GATED_SCREENS = ["forms", "safetyplan", "toolbox", "workplans"];
