...
// ---- Screen: Home ----
function HomeScreen({ go, progress }) {
  const { sheetUrls, fallbacks, name: projectName } = useContext(ProjectContext);
  const sections = [
  ...
  const [posts, loadingPosts] = useSheetData(sheetUrls.bulletin, fallbacks.bulletin, (row) => ({
  ...
  const [emergencyContacts] = useSheetData(sheetUrls.emergencyContacts, fallbacks.emergencyContacts, (row) => ({
...
const HS_STANDARDS_ITEM = { title: "H&S Minimum Standards", pages: 38, link: "" };
// The Safety Plan itself is project-specific — each project's fallback is set in PROJECTS below.
const SAFETY_PLAN_DEFAULT = [{ title: "Safety Plan", pages: 135, link: "" }];
...
function ManualScreen() {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="text-[12px] mb-3" style={{ color: STEEL }}>
        View the full safety manual by section. Viewing only — sections cannot be downloaded.
      </div>
      ...
      <div className="space-y-2">
        {sections.map((s, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-1" style={{ color: STEEL }}>
              <Eye size={17} />
            </div>
          </button>
        ))}
...
const FORMS_DEFAULT = [
    ...
    statusGroups: [
      {
        ...
        options: ["Y", "N/A"],
        items: [
          "Acceptable entry conditions defined?",
          "Plan to monitor and test work area?",
          "Plan to isolate release of energy and material into work area?",
          "Plan to eliminate or control atmospheric hazards in work area?",
          "Plan to protect workers from external hazards?",
          "Plan to verify conditions are acceptable for the entire duration?",
        ],
      },
      {
  ...
  },
  {
    ...
    fields: ["Segment", "Operator", "Project Location", "Equipment Type", "Supervisor", "Remarks"],
    statusGroups: [
      {
        label: "Inspection Items",
        options: ["OK", "Bad", "N/A"],
        items: [
          "Tracks, tires", "Brakes", "Horn", "Lights", "Windshield Wiper", "Glass",
          "Rear View Visibility", "Body", "Covers", "Roll Over Protection", "Dump Mechanism",
          "Steering – Control", "Fire Extinguisher", "Signal System", "Fuel & Gas Lines",
          "Fuel Tank", "Exhaust System", "Back up Alarm", "Seatbelt", "Grab Bars, Steps",
          "Warning Lights", "Motor / Wiring", "Radiator", "Belts", "Hoses", "Overhead Rotating Beacon",
        ],
      },
    ],
  },
  {
    title: "Job Hazard Analysis – Life Saving Controls",
    fields: [
      "Segment", "Work Plan #", "Competent Person Name", "Supervisor Name",
      "Certified FA/CPR Person", "Certified Flagger", "Task",
      "Machinery Used", "Tools Used", "Special PPE Required",
    ],
    stepPlan: {
      minSteps: 3,
      fieldLabels: ["Step to Complete Work", "Hazards Associated With This Step", "Control Measures", "Lifesaving Control"],
    },
    statusGroups: [
      {
  ...
  {
    title: "Utility Locate Permit (Excavation / Drilling / Boring)",
    fields: [
      "Requested By", "Competent Person", "Segment #", "Start Date", "Completion Date",
      "Purpose of Excavation / Drilling / Boring", "Location of Excavation / Drilling / Boring",
      "Depth", "Width / Diameter", "Length",
      "Utility Locates Completed (type, reference #, date located)",
      "Ticket Number(s)",
    ],
    choiceGroups: [
      ...
      <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/80 p-4">
        <div className="bg-white rounded-md w-full max-w-sm p-6 text-center">
          ...
          <div className="text-[12px] mt-1" style={{ color: STEEL }}>
            Emailed to your designated inbox as a signed PDF.
          </div>
...
function AddPersonScreen({ user, onDone }) {
  ...
  const submit = async () => {
    setSubmitting(true);
    setError("");
    const photoUrl = await uploadPhoto(employeeId.trim());
    ...
    <div className="bg-white rounded-md p-3 border mb-2" style={{ borderColor: "#E4E2DA", opacity: person.active ? 1 : 0.55 }}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-[11px]" style={{ color: STEEL }}>ID {person.employee_id} · Badge {person.badge_number}{!person.active ? " · Inactive" : ""}</div>
        </div>
...
const ORIENTATION_DOCS_DEFAULT = [
  { title: "Competent Person Designation", pdfLink: "" },
  { title: "Equipment Operator Designation", pdfLink: "" },
  { title: "Drug Screen Affidavit", pdfLink: "" },
];

function OrientationScreen() {
  const { sheetUrls, fallbacks } = useContext(ProjectContext);
  const [sessions, loadingSessions] = useSheetData(sheetUrls.orientation, fallbacks.orientation, (row) => ({
    date: row.Date || "", time: row.Time || "", location: row.Location || "", notes: row.Notes || "",
  }));
  const [docs] = useSheetData(sheetUrls.orientationDocs, ORIENTATION_DOCS_DEFAULT, (row) => ({
    title: row.Title || "Untitled", pdfLink: row.PdfLink || "",
  }));
  const [downloaded, setDownloaded] = useState({});
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="text-[11px] uppercase font-bold mb-2" style={{ color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>Upcoming Sessions</div>
      ...
      <div className="text-[11px] uppercase font-bold mb-2" style={{ color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>What To Bring - Orientation Documents</div>
      <div className="space-y-2 mb-5">
        {docs.map((d, i) => (
          <div key={i} className="bg-white rounded-md p-3 border flex items-center justify-between" style={{ borderColor: "#E4E2DA" }}>
            <div className="flex items-center gap-2 min-w-0">
            ...
            <button onClick={() => handleDownload(d, i)} className="flex items-center gap-1 text-[10px] font-bold uppercase px-2 py-1.5 rounded-sm border flex-shrink-0 ml-2" style={{ borderColor: "#C9C6BC", color: STEEL, fontFamily: "IBM Plex Mono, monospace" }}>
              {downloaded[i] ? <CheckCircle2 size={13} color={GOLD} /> : <Download size={13} />} {downloaded[i] ? "Saved" : "Download"}
            </button>
          </div>
        ))}
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="md:max-w-3xl md:mx-auto md:w-full">
            {screen === "home" && <HomeScreen go={go} progress={progress} />}
            ...
            {screen === "workplans" && <WorkPlansScreen />}
            ...
            {screen === "orientation" && <OrientationScreen />}
          </div>
        </div>
