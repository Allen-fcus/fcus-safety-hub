...
const BUILD_VERSION = "debug-drugscreen-v7";
...
async function fillDrugScreenPdf({ templateUrl, companyName, companyAddress, companyCityStateZip, recipientName, date, employeeNames, contactName, contactPhone, logoDataUrl }) {
  ...
  try {
    ...
    // filled, not removed. removeField() is a known source of dangling
    // references in some PDF libraries that only break later, at save
    // time. An untouched, empty text field has no visible border/fill
    // by default, so drawing the logo image on top of it (below) covers
    // it visually without needing to modify the field at all.
