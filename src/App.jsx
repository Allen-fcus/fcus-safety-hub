...
// Bump this string with every deploy during active debugging, so it's
// possible to visually confirm — right in the app, no GitHub/Vercel
// digging required — exactly which build is actually running live.
const BUILD_VERSION = "debug-drugscreen-v5";
...
const ORIENTATION_FORMS_DEFAULT = [
  {
    ...
    pdfLink: "https://kzsmchyowjueksdivsaa.supabase.co/storage/v1/object/public/documents/drug-screen-affidavit-template.pdf",
...
async function fillDrugScreenPdf({ templateUrl, companyName, companyAddress, companyCityStateZip, recipientName, date, employeeNames, contactName, contactPhone, logoDataUrl }) {
  let step = "STEP_0_INIT";
  try {
    step = "STEP_1_FETCH_TEMPLATE";
    const res = await fetch(templateUrl);
    if (!res.ok) throw new Error("Could not download the blank form template.");
    const templateBytes = await res.arrayBuffer();

    step = "STEP_2_LOAD_PDF";
    const pdfDoc = await PDFDocument.load(templateBytes);

    step = "STEP_3_GET_FORM";
    const form = pdfDoc.getForm();

    step = "STEP_4_SET_TEXT_FIELDS";
    const setText = (name, value) => {
      try { form.getTextField(name).setText(value || ""); } catch (e) { /* field missing — skip */ }
    };
    // "Your Logo Here" is handled separately below (removed entirely,
    // then a real uploaded image is drawn in its place).
    setText("Date", date);
    ...
    step = "STEP_4B_REMOVE_LOGO_PLACEHOLDER_FIELD";
    try {
      form.removeField(form.getTextField("Your Logo Here"));
    } catch (e) {
      // Field already gone or never existed — fine either way.
    }

    // Flattening is skipped entirely for this form — the filled-in values
    // still display correctly in any PDF viewer either way, and the
    // flatten step (or its fallback retry) appears to leave this specific
    // document in a state that only breaks later, at save() time.
    step = "STEP_5_SKIP_FLATTEN";

    if (logoDataUrl) {
      step = "STEP_6_CHECK_LOGO_FORMAT";
      ...
      step = "STEP_7_READ_LOGO_BYTES";
      const buf = await fetch(logoDataUrl).then((r) => r.arrayBuffer());
      const imgBytes = new Uint8Array(buf);

      step = "STEP_8_EMBED_LOGO_IMAGE";
      const logoImage = isJpeg ? await pdfDoc.embedJpg(imgBytes) : await pdfDoc.embedPng(imgBytes);

      step = "STEP_9_GET_PAGE";
      const page = pdfDoc.getPage(0);

      step = "STEP_10_DRAW_LOGO_IMAGE";
      const boxX = 191.336, boxY = 720.168, boxW = 411.336 - 191.336, boxH = 742.168 - 720.168;
...
function FillFormModal({ form, onClose, user }) {
  ...
  const doSubmit = async () => {
    ...
    if (form.pdfFillTemplate === "drug-screen-affidavit") {
      ...
      setPdfError("");
      let callerStep = "CALLER_STEP_0_START";
      try {
        callerStep = "CALLER_STEP_1_BUILD_INDEXES";
        ...
        callerStep = "CALLER_STEP_2_CALL_FILL_FUNCTION";
        ...
        callerStep = "CALLER_STEP_3_MAKE_BLOB";
        const blob = new Blob([filledBytes], { type: "application/pdf" });

        callerStep = "CALLER_STEP_4_TRIGGER_DOWNLOAD";
        ...
        callerStep = "CALLER_STEP_5_UPLOAD_TO_STORAGE";
        ...
        if (!uploadError) {
          callerStep = "CALLER_STEP_6_GET_PUBLIC_URL";
          const { data: urlData } = supabase.storage.from("documents").getPublicUrl(path);
          filledPdfUrl = urlData && urlData.publicUrl ? urlData.publicUrl : null;
        }
        setPdfGenerated(true);
      } catch (e) {
        setPdfError(`(${callerStep}) ${e && e.message ? e.message : "Could not generate the filled PDF."}`);
      }
    ...
    return (
      <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/80 p-4">
        <div className="bg-white rounded-md w-full max-w-sm p-6 text-center">
          ...
          <div className="text-[9px] mt-2" style={{ color: "#9AA0A6", fontFamily: "IBM Plex Mono, monospace" }}>Build: {BUILD_VERSION}</div>
...
const ORIENTATION_DOCS_DEFAULT = [
  ...
  { title: "Drug Screen Affidavit", pdfLink: "https://kzsmchyowjueksdivsaa.supabase.co/storage/v1/object/public/documents/drug-screen-affidavit-template.pdf" },
];
