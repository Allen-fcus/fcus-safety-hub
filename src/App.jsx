...
function OrientationScreen({ user, go, activeProjectId }) {
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      {user && user.canAddPersonnel && (
        <button onClick={() => go("manageorientationsignins")} className="w-full mb-4 flex items-center justify-center gap-1 text-[11px] font-bold uppercase py-2.5 rounded-md" style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}>
          {t("reviewSignIns")}
        </button>
      )}
      <div className="flex items-center justify-between mb-2">
...
function BadgeModal({ user, onClose, trainingRecords, onLogout }) {
  ...
  const addToGoogleWallet = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.saveUrl) throw new Error(data.error || "Could not create the pass.");
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/85 px-6">
      <div className="w-full max-w-xs rounded-xl overflow-hidden max-h-[90vh] flex flex-col" style={{ background: "white", boxShadow: "0 10px 40px rgba(0,0,0,0.4)" }}>
        ...
        <div className="p-5 flex flex-col items-center overflow-y-auto">
          ...
          {walletError && <div className="text-[10px] mt-1.5 text-center px-2" style={{ color: ALERT }}>{walletError}</div>}
          {onLogout && (
            <button
              onClick={onLogout}
...
const ALWAYS_PUBLIC_SCREENS = ["emergency", "concern", "laborlaws", "orientationsignin", "hsstandards"];
