...
const DESIGNATION_PDF_CONFIGS = {
  ...
  "emergency-contacts-list": {
    // No fixed templateUrl — each project uploads its own copy via
    // Pre-Construction → Manage, so this is always supplied at fill
    // time via form.templateUrlOverride instead.
    buildTextFields: ({ standardValues, getField }) => ({
      Date7_af_date: standardValues.date,
  ...
  "safety-requirements-summary": {
    // Same per-project template pattern as Emergency Contacts List.
...
// Only these two are fill-and-sign — Work Plan Template and Man Hour
// Work Submittal stay view/download only, per each project's own
// uploaded template (set via templateUrlOverride at open time).
            ...
            <div key={title} className="bg-white rounded-md p-3 border" style={{ borderColor: "#E4E2DA" }}>
              ...
              {doc && doc.doc_link ? (
                <div className="flex gap-2">
                  {fillableBase ? (
                    <button
                      onClick={() => setFilling({ ...fillableBase, pdfLink: doc.doc_link, templateUrlOverride: doc.doc_link })}
