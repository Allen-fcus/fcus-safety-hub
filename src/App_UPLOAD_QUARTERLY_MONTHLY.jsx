...
const PRECONSTRUCTION_DOC_TITLES = ["Safety Requirements Summary", "Emergency Contacts List", "Work Plan Template", "Man Hour Work Submittal"];

// These two are fixed, fill-and-sign templates — same pattern as any
// other form in Forms & Templates, no per-project upload needed.
// Work Plan Template and Man Hour Work Submittal stay view/download
// only, each project uploading its own via Manage.
...
// Man Hour Work Submittal isn't a fixed template like the two above —
// each project uploads its own via Manage, so templateUrlOverride gets
// set at open time from that project's uploaded file.
const MAN_HOUR_REPORT_FORM = {
    ...
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      ...
      <div className="space-y-2">
        {PRECONSTRUCTION_DOC_TITLES.map((title) => {
          ...
          // Work Plan Template / Man Hour Work Submittal — each project
          // uploads its own copy via Manage.
            ...
            <div key={title} className="bg-white rounded-md p-3 border" style={{ borderColor: "#E4E2DA" }}>
              ...
              {doc && doc.doc_link ? (
                <div className="flex gap-2">
                  {title === "Man Hour Work Submittal" && (
                    <button
                      onClick={() => setFilling({ ...MAN_HOUR_REPORT_FORM, pdfLink: doc.doc_link, templateUrlOverride: doc.doc_link })}
                      className="flex-1 text-[10px] font-bold uppercase py-1.5 rounded-sm"
                      style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}
                    >
                      Fill Out
                    </button>
