...
function PreOrientationFormStep({ form, projectId, activeProjectId, scheduleId, defaultSubcontractor, stepLabel, onDone }) {
  ...
  const submit = async () => {
    ...
    const { data, error: rpcError } = await supabase.rpc("submit_pre_orientation_form", {
      ...
      p_data: { subcontractor: defaultSubcontractor, fields: values, statusValues, choiceValues },
    });
    setSubmitting(false);
    ...
    if (notifyEmail || form.submitEmail) {
      sendFormEmail({
        ...
        fieldsText: buildFieldsText(),
        submittedBy: defaultSubcontractor,
...
function OrientationScheduleScreen({ activeProjectId }) {
  ...
  const startSchedule = async () => {
    ...
    // Build the sequence: Drug Screen Affidavit always first, then each
    // checked designation, in the same order they appear in Orientation.
    const titleOrder = [
      "Drug Screen Affidavit",
    ...
    const queue = titleOrder.map((title) => ORIENTATION_FORMS_DEFAULT.find((f) => f.title === title)).filter(Boolean);
    setFormQueue(queue);
    setFormIndex(0);
    ...
    return (
      <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
        ...
        <PreOrientationFormStep
          ...
          stepLabel={`Form ${formIndex + 1} of ${formQueue.length}`}
          onDone={advanceForm}
        />
      </div>
    );
...
function ManageOrientationSchedulesScreen({ user }) {
  ...
  const expectedForms = (s) => {
    const list = ["Drug Screen Affidavit"];
    if (s.needs_competent_person) list.push("Competent Person Designation");
    if (s.needs_equipment_operator) list.push("Equipment Operator Designation");
    if (s.needs_crane_operator) list.push("Crane Operator Designation");
    if (s.needs_ad_director) list.push("Assembly/Disassembly (A/D) Director Designation");
    if (s.needs_flagger) list.push("Flagger Designation");
    return list;
  };
  ...
  return (
    <div className="px-4 pt-4 pb-8" style={{ background: "#F4F3EF", minHeight: "100%" }}>
      <div className="text-[12px] mb-3" style={{ color: STEEL }}>
        Everyone who's scheduled an orientation, and which forms they've actually completed so far.
      </div>
      {loading && <div className="text-[12px] mb-2" style={{ color: STEEL }}>Loading…</div>}
      {schedules.map((s) => {
        const expected = expectedForms(s);
        const done = s.forms_submitted || [];
        return (
          <div key={s.id} className="bg-white rounded-md p-3 border mb-2" style={{ borderColor: "#E4E2DA" }}>
            ...
            <div className="mt-2 space-y-1">
              {expected.map((title) => (
                <div key={title} className="flex items-center gap-1.5 text-[11px]">
                  {done.includes(title) ? <CheckCircle2 size={13} color="#1E7A34" /> : <Circle size={13} color={STEEL} />}
                  <span style={{ color: done.includes(title) ? INK : STEEL }}>{title}</span>
                </div>
              ))}
            </div>
          </div>
...
const ORIENTATION_FORMS_DEFAULT = [
  ...
  },
  {
    ...
    description: "All employees who will operate heavy equipment on-site are required to submit a completed Equipment Operator Designation Form prior to performing any equipment-related work. This form documents the employee's authorization to operate designated equipment. Employees who will operate a forklift, skytrak, crane, or manlift must also submit current training certifications for each applicable piece of equipment along with the designation form. Failure to provide the required documentation will result in the employee being restricted from operating equipment on-site until all requirements are met.",
    submitEmail: "ntisafety@ferrovial.us",
...
// Equipment Operator) — just text fields and checkboxes, no
// image overlay needed. Signature fields are deliberately never
// touched (same lesson learned from Drug Screen Affidavit — these
// are real /Sig placeholders, not something we can draw into, and
// leaving them alone avoids the save() issues we hit before).
// ------------------------------------------------------------
async function fillDesignationPdf({ templateUrl, textFields, checkboxFields }) {
  const res = await fetch(templateUrl);
  if (!res.ok) throw new Error("Could not download the blank form template.");
  const templateBytes = await res.arrayBuffer();
  ...
  const doSubmit = async () => {
    ...
    if (DESIGNATION_PDF_CONFIGS[form.pdfFillTemplate]) {
      ...
      try {
        ...
        const ctx = { standardValues, getField, statusValues, choiceValues, form };
    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/80 p-4">
      ...
      >
        ...
        <div className="p-4 space-y-4 overflow-y-auto">
          ...
          <button
            disabled={sigEmpty || submitting || (form.requiresProof && form.proofRequired !== false && proofFiles.length === 0) || (form.requiresLogo && !logoFile)}
            onClick={doSubmit}
