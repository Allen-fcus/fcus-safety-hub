...
function OrientationSignInScreen({ activeProjectId }) {
  ...
  const submit = async () => {
    ...
    if (!projectUuid) {
      setSubmitting(false);
      setError("Couldn't determine which project this is for. Please rescan the QR code.");
      return;
    }
...
function OrientationScheduleScreen({ activeProjectId }) {
  ...
  const startSchedule = async () => {
    ...
    const { data, error: rpcError } = await supabase.rpc("create_orientation_schedule", {
      p_project_slug: activeProjectId,
      p_session_day: day,
    ...
    setSubmitting(false);
    if (rpcError || !data || !data.success) { setError("Something went wrong. Please try again."); return; }
      ...
      {editing && (
        <div className="mt-3 pt-3 border-t" style={{ borderColor: "#E4E2DA" }}>
            ...
            {quals.map((q) => (
              <div key={q.id} className="flex items-center justify-between bg-white rounded-sm px-2.5 py-1.5 border" style={{ borderColor: "#E4E2DA" }}>
                <div className="flex-1 min-w-0">
                  ...
                  {q.expires_at && <div className="text-[10px] mt-0.5" style={{ color: STEEL }}>Expires {new Date(q.expires_at + "T00:00:00").toLocaleDateString()}</div>}
                </div>
          ...
          <div className="bg-white rounded-md p-2 border mb-3" style={{ borderColor: "#E4E2DA" }}>
            <input value={newQualLabel} onChange={(e) => setNewQualLabel(e.target.value)} placeholder="e.g. Competent Person – Fall Protection" className="w-full rounded-md border px-2 py-1.5 text-[12px] mb-1.5" style={{ borderColor: "#C9C6BC" }} />
            <div className="flex gap-2">
              <input type="date" value={newQualExpiry} onChange={(e) => setNewQualExpiry(e.target.value)} className="flex-1 rounded-md border px-2 py-1.5 text-[12px]" style={{ borderColor: "#C9C6BC" }} />
              <button onClick={addQual} disabled={addingQual} className="text-[10px] font-bold uppercase px-3 rounded-sm disabled:opacity-50" style={{ background: GOLD, color: INK, fontFamily: "IBM Plex Mono, monospace" }}>
                {addingQual ? "…" : "Add"}
              </button>
            </div>
            <div className="text-[10px] mt-1" style={{ color: STEEL }}>Leave date blank for designations that don't expire.</div>
          </div>
...
function getInitialProjectFromUrl() {
  try {
    ...
    return p && PROJECTS[p] ? p : "safety-hub";
  } catch (e) {
    return "safety-hub";
  }
}
